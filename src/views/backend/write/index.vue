<template>
    <section class="workspace__wrapper">
            ...
            >
                <a-form-item>
                    <a-button
                        ...
                        @click.prevent="onClickPublish"
                        >{{ isEdit ? '保存博客' : '发布博客' }}</a-button
                    >
                </a-form-item>
                <a-form-item name="poster" label="封面url">
                    ...
                    <a-image :width="200" :src="formModel.poster" />
                </a-form-item>
        ...
        <a-modal
            v-model:visible="isRelationVisible"
            title="关联分类/标签"
            :footer="null"
            width="860px"
        >
            ...
            >
                ...
                <a-form-item name="newCategorys" label="新增分类">
                    <a-row :gutter="16">
                        <a-col
                            v-for="(category, index) in newCategoryList"
                            :key="index"
                            :xs="8"
                            :sm="4"
                        >
                    ...
                    <a-checkbox-group v-model:value="relFormModel.oldCategoryIds">
                        <a-row :gutter="16">
                            <a-col v-for="category in categorys" :key="category.id" :xs="8" :sm="4">
                                <a-checkbox class="category-checkbox" :value="category.id">{{
                ...
                <a-form-item class="align-center">
                    <a-button
                        ...
                        @click.prevent="onConfirmPublish"
                        >{{ isEdit ? '确认保存' : '确认发布' }}</a-button
                    >
                </a-form-item>
            </a-form>
        </a-modal>
...
<script setup lang="ts" name="BackendArticle">
    ...
    const userInfo = computed<UserDTO | null>(() => globalStore.userInfo);
    ...
    const originalTagNames = computed(
        () => articleDetail.value?.tags.map((item) => item.tagName) || []
    );
    const originalCategoryIds = computed(
        () => articleDetail.value?.categories.map((item) => item.id) || []
    );
    const init = async () => {
        if (route.params.id) {
            isEdit.value = true;
    ...
    const addTag = () => {
        if (newTagList.value.length > 0) {
            ...
            if (emptyExsited) {
                // 如果有没填写的标签，提示必须填写
                globalProxy.$message.warning('请先输入已添加的标签内容');
            } else if (newTagList.value.length >= 4) {
                // 限制标签数量
                globalProxy.$message.warning('最多添加4个标签');
            } else {
    ...
    const addCategory = () => {
        if (newCategoryList.value.length > 0) {
            ...
            if (emptyExsited) {
                // 如果有没填写的标签，提示必须填写
                globalProxy.$message.warning('请先输入已添加的分类');
            } else if (newCategoryList.value.length >= 4) {
                // 限制标签数量
                globalProxy.$message.warning('最多添加4个分类');
            } else {
    ...
    const handleConfirmPublish = async () => {
        // 计算出所有的标签名
        const tagNames = newTagList.value
            .filter((item) => item.value.trim() !== '')
            .map((item) => item.value.trim());
        if (tagNames.length === 0) {
            globalProxy.$message.warning('至少输入一个标签');
            return Promise.reject(false);
        }

        // 计算出输入的新的分类名
        const newCategoryNames = newCategoryList.value
            .filter((item) => item.value.trim() !== '')
            .map((item) => item.value.trim());
        ...
        if (isRepeatCategoryName) {
            globalProxy.$message.warning(
                `新增的分类名称[${repeatCategoryName}]已经存在，请删除后直接在已有分类中选择`
            );
            return Promise.reject(false);
        }
        ...
        if (newCategoryNames.length === 0 && relFormModel.oldCategoryIds.length === 0) {
            globalProxy.$message.warning('至少选择一个分类');
            return Promise.reject(false);
        }
        ...
        if (isEdit.value) {
            ...
            globalProxy.$message.success('保存成功');
        } else {
            // 新建场景
            const params = {
                ...
                ...formModel,
                oldCategoryIds:
                    relFormModel.oldCategoryIds.length > 0 ? relFormModel.oldCategoryIds : null,
                tags: tagNames,
                newCategories: newCategoryNames.length > 0 ? newCategoryNames : null
            };
            await articleService.add(params);
            globalProxy.$message.success('创作成功');
        }

        router.push('/backend');
    };

    const { trigger: onConfirmPublish, loading: isConfirmLoading } =
        useAsyncLoading(handleConfirmPublish);
...
<style lang="less" scoped src="@/views/frontend/article/md-render.less"></style>
