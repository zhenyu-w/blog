    ...
    export default defineComponent({
        ...
        data() {
            return {
                ...
                triggerType: 1 // 1代表enter触发 2代表ctrl+enter触发
            };
        },
        watch: {
            msgList: {
        ...
        methods: {
            createSocket() {
                const self = this;
                this.socket = io(import.meta.env.VITE_APP_SOCKET_SERVER);
            ...
            async sendChatContent() {
                const self = this;
                await this.$refs.chatForm.validate();
                this.socket.emit('chat', self.chatForm.chatContent);
                this.msgList.push({
                    time: format(new Date(), 'HH:mm:ss'),
                    user: '我说',
                ...
                this.$refs.chatForm.resetFields();
            },
            onKeydownChat(e) {
                if (
                    (this.triggerType === 1 && e.keyCode === 13) ||
                    (this.triggerType === 2 && e.ctrlKey && e.keyCode === 13)
                ) {
                    e.preventDefault();
